...
class EventProcessorService2Test {
    ...
    void testProcessEvents() {
        ...
        Flux<Event> eventFlux = eventProcessorService.processEvents(Flux.fromIterable(events)).cache();

        // -------------------------------------------------------
        // 검증
        // -------------------------------------------------------
        StepVerifier.create(eventFlux).expectNextCount(1000).verifyComplete();

        // 마지막 결과값이 103 = 100 + 3 인지 확인
        ArgumentCaptor<Result> argumentCaptor = ArgumentCaptor.forClass(Result.class);
        verify(publishResultPort, times(100)).publishResult(argumentCaptor.capture());
        assertEquals(argumentCaptor.getAllValues().get(99).getResult(), "103");

        // 연산 100번, 배치 쓰기 4번, 이벤트 읽기 1번
